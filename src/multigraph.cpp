...
void Multigraph2::addArc(int i, int j, std::vector<int> ws){
    if (i>=N || i<0 ||j>=N || j<0 ) {
            print_and_exit("addEdge : La valeur des sommets n'est pas correcte");
        }
        if (static_cast<int>(ws.size()) != dim) {
...
Multigraph2 Multigraph2::load_graph(std::string path){
    std::ifstream file(path);
    if (!file.is_open()) {
        print_and_exit("load_graph : can't open the file");
    }
    ...
    Multigraph2 g = Multigraph2(dim, N);
